...
def _fmt_offer_line(o: Offer) -> str:
    ...
    return (f"  • {o.price:>8.2f} €  {o.model}  [{o.ram}GB/{o.storage}GB "
            f"{o.color} {o.condition}{kb}]{av}\n    {o.url}")


def render_text(report: Report, new_sigs: set, ts: str) -> str:
    L: list[str] = []
...
def subject_line(report: Report, new_sigs: set) -> str:
    return (f"🟢 Refurbed: {len(new_sigs)} novih "
            f"({len(report.anomalies)} anomalija ukupno)")
